...
from models import Post, Outline, Section, SEOAnalysis, MetaTag
...
async def run_seo_analysis(post_id: int, db: AsyncSession = Depends(get_db)):
...
async def get_seo_analyses(post_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(SEOAnalysis)
...
async def generate_meta_tags(post_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(Post)
        .where(Post.id == post_id)
        .options(
    ...
    # Store title variations in latest SEO analysis
...
async def get_meta_tags(post_id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(MetaTag)
